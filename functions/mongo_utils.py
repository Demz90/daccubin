...
def initialise_mongo_cloud_db_client():
    ...
    mongo_server_public_ip_address = os.environ.get('utom_mongo_server_public_ip_address')
    mongodb_server_username = os.environ.get('utom_mongo_server_username')
    mongodb_server_password = os.environ.get('utom_mongo_server_password')
...
def input_document_list_into_mongo_db_collection(client, db_name, collection_name, document_list):
    ...
    Returns:
        list | None: A list of the inserted ObjectId instances, or None if an error occurs.
    """
    try:
        # Connect to the specific collection
        db = client[db_name]
        ...
        # Add data to the collection and capture the result
        insert_result = collection.insert_many(document_list)
        
        # Return the list of inserted ObjectIds
        if insert_result and insert_result.inserted_ids:
            logging.info(f"Successfully inserted {len(insert_result.inserted_ids)} documents into {db_name}.{collection_name}")
            return insert_result.inserted_ids
        else:
            # This case might occur if document_list was empty or invalid before insert_many
            logging.warning(f"insert_many operation did not return inserted_ids for {db_name}.{collection_name}. Document list might have been empty.")
            return [] # Return empty list if successful but no IDs (e.g., empty input list)
            
    except Exception as e:
        logging.error(f"Error inserting multiple documents into {db_name}.{collection_name}: {e}", exc_info=True)
...
def update_mongo_collection_document_by_obj_id(client, db_name, collection_name, content_id, input_data_dict) -> bool:
    """
    Update specific fields of a document in a MongoDB collection identified by its ObjectId.

    Args:
        client (MongoClient): A MongoDB client instance.
        db_name (str): The name of the database.
        collection_name (str): The name of the collection.
        content_id (ObjectId): The ObjectId of the document to update.
        input_data_dict (dict): A dictionary containing the key-value pairs to update using $set.

    Returns:
        bool: True if a document was matched and potentially modified, False otherwise (not found or error).
    """
    try:
        db = client[db_name]
        collection = db[collection_name]

        # Define the query to find the document by its ObjectId
        query = {"_id": content_id}
        
        # Define the update operation using the $set operator
        update_operation = {"$set": input_data_dict}

        logging.info(f"Attempting to update document with _id: {content_id} in {db_name}.{collection_name} using $set.")
        ...
        if result.matched_count > 0:
            if result.modified_count > 0:
                logging.info(f"Successfully updated document with _id: {content_id}. Modified count: {result.modified_count}")
            else:
                logging.info(f"Document with _id: {content_id} matched but no fields needed modification.")
            return True # Return True if matched (even if no fields changed)
        else:
            logging.warning(f"No document found with _id: {content_id} to update in {db_name}.{collection_name}.")
            return False

    except Exception as e:
        logging.error(f"Error updating document with _id {content_id} in {db_name}.{collection_name}: {e}", exc_info=True)
        return False
...
def find_documents_by_id(mongo_client, db_name, collection_name, document_id_key_name, document_id_str, projection=None):
    """
...
def delete_document_from_mongo_db_collection_by_object_id(client, db_name, collection_name, object_id: ObjectId) -> bool:
    """
    Deletes a single document from a MongoDB collection based on its ObjectId.

    Args:
        client (MongoClient): A MongoDB client instance.
        db_name (str): The name of the database.
        collection_name (str): The name of the collection.
        object_id (ObjectId): The ObjectId of the document to delete.
    ...
    try:
        ...
        if result.deleted_count == 1:
            # print(f"Successfully deleted document with _id: {object_id}")
            return True
        else:
            # print(f"No document found with _id: {object_id} or delete was unsuccessful.")
            return False
    except Exception as e:
        # print(f"Error deleting document with _id {object_id}: {e}")
        return False

def delete_many_documents_by_key_and_search_term(client, db_name, collection_name, key_name: str, search_term: str) -> int:
    """
    Deletes multiple documents from a MongoDB collection based on a key and search term.

    Args:
        client (MongoClient): A MongoDB client instance.
        db_name (str): The name of the database.
        collection_name (str): The name of the collection.
        key_name (str): The key to filter documents by.
        search_term (str): The value to match for the given key.

    Returns:
        int: The number of documents deleted. Returns -1 if an error occurs.
    """
    try:
        db = client[db_name]
        collection = db[collection_name]
        ...
        delete_filter = {key_name: search_term}
        
        # Perform the delete operation
        result = collection.delete_many(delete_filter)
        
        # print(f"Successfully deleted {result.deleted_count} documents matching key '{key_name}' and search term '{search_term}'.")
        return result.deleted_count
    except Exception as e:
        # print(f"Error deleting documents matching key '{key_name}' and search term '{search_term}': {e}")
        return -1

from typing import *
from bson import ObjectId

def search_and_upsert_key_value(
    collection,
    query_dict: Dict[str, str],
    upsert_dict: Dict[str, str]
):
    """
    Searches for documents matching {search_key: search_value} and adds a new
    {upsert_key: upsert_value} to each matching document without replacing existing fields.

    Args:
        collection: The PyMongo collection object.
        search_key (str): The key to filter documents.
        search_value: The value of the key to search for.
        upsert_key (str): The key to add/update in the matching documents.
        upsert_value: The value to set for the upsert_key.
    """
    query = query_dict
    update = {"$set": upsert_dict}

    matching_docs = list(collection.find(query))

    if not matching_docs:
        print("No match found. Inserted new document.")
        return
...
def get_documents_by_key(
        collection, key: str, 
        id_list: List[Union[str, ObjectId]],
        projection: Dict[str, int] = None) -> List[dict]:
    
    """
    Retrieve multiple documents from a MongoDB collection using a list of _id values.

    Args:
        collection: A pymongo collection instance.
        id_list (List[Union[str, ObjectId]]): List of document IDs (as strings or ObjectIds).

    Returns:
        List[dict]: A list of documents matching the given IDs.
    """

    cursor = collection.find(
        {key: {"$in": id_list}}, projection)
    return list(cursor)